...
const ProcessedData = () => {
  ...
  const fetchProcessedFiles = async () => {
        ...
        try {
            ...
            const res = await axios.get('http://192.168.1.11:5000/processed-files', {
  ...
  const handleDownload = (fileName) => {
    ...
    window.open(`http://192.168.1.11:5000/download/processed/${fileName}`, '_blank');
  };
  ...
  const handleView = async (file) => {
      ...
      if (file.type === 'Image') {
          setPreviewPanel(prev => ({
              ...prev, loading: false, imageSrc: `http://192.168.1.11:5000/download/processed/${file.name}`
          }));
          return;
      }
      ...
      try {
          const token = localStorage.getItem('token');
          const res = await axios.get(`http://192.168.1.11:5000/processed-files/${file.id}/preview`, {
              headers: { Authorization: `Bearer ${token}` }
          });
  ...
  const confirmDelete = async () => {
    ...
    try {
        const token = localStorage.getItem('token');
        await axios.delete(`http://192.168.1.11:5000/processed-files/${id}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
